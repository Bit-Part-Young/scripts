...
# 可忽略，因其切变量 S 最大，为 1.890，其孪生最难发生
# 从构型的角度看，其最小原子间距小于 1.5 Å，原子受力会很大
#------------------------- 构建 HCP {11-23} 孪晶模型 --------------------------
function twin_11_23_generation() {
  ...
  atomsk --create hcp ${a} ${c} ${symbol} orient "[1-100]" "[11-22]" "[11-23]" -duplicate ${dup_x} ${dup_y} ${dup_z2} -sort z up -wrap -ow vasp

  mv POSCAR bulk_${symbol}.vasp

  atomsk --create hcp ${a} ${c} ${symbol} orient "[1-100]" "[11-22]" "[11-23]" -duplicate ${dup_x} ${dup_y} ${dup_z} -ow ${symbol}_cell.xsf
  atomsk --create hcp ${a} ${c} ${symbol} orient "[1-100]" "[11-22]" "[-1-12-3]" -duplicate ${dup_x} ${dup_y} ${dup_z} -ow ${symbol}_mirror.xsf
  atomsk --merge Z 2 ${symbol}_mirror.xsf ${symbol}_cell.xsf -ow ${symbol}_final.cfg
  atomsk ${symbol}_final.cfg -sort z up -wrap -ow vasp

  rm *.xsf *.cfg

  mv POSCAR twin_11_23_${symbol}_original.vasp
