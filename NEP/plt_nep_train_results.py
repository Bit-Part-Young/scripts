"""
绘制 NEP 训练 loss 演化与能量、力、virial RMSE 指标 DFT计算值 NEP 预测值对比图
...
axs[0, 0].legend(
    ["Total", "L1-Reg", "L2-Reg", "Energy-train", "Force-train", "Virial-train"], prop={"size": 8}
)
axs[0, 0].axis("tight")
...
axs[1, 0].plot(
    [xmin_force, xmax_force], [xmin_force, xmax_force], linewidth=2, color="grey", linestyle="--"
)
...
axs[1, 0].text(
    0.5,
    0.08,
    rf"RMSE: {mean_force_rmse:.2f} meV/$\AA$",
...
axs[1, 1].text(
    0.5,
    0.08,
    f"RMSE: {mean_stress_rmse:.4f} GPa",
