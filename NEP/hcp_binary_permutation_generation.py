...
"""
生成 HCP 2x2x2 超胞（16 个原子）的二元固溶体的不同浓度的对称性非等同的置换构型
每种 HCP 二元固溶体中每种浓度至多保留 20 个构型，共 110 个置换构型
...
def get_hcp_binary_lc(natoms_list: list, element_list: list):
    # [ ] HCP 2x2x2 这样计算是否合适（a c 分别取平均）？
    """获取 HCP 2x2x2 超胞的二元合金不同浓度的平均晶格常数"""

    lc_a = (
        element_lc_dict[element_list[0]][0] * natoms_list[0]
        + element_lc_dict[element_list[1]][0] * natoms_list[1]
    ) / sum(natoms_list)

    lc_c = (
        element_lc_dict[element_list[0]][1] * natoms_list[0]
        + element_lc_dict[element_list[1]][1] * natoms_list[1]
    ) / sum(natoms_list)

    return round(lc_a, 3), round(lc_c, 3)
...
def enumerate_solution(element_initial: str, element_replace: str, num_kept: int = 20):
    ...
    natoms = len(bulk("Ti", "hcp", a=2.936, c=4.646) * (2, 2, 2))
    # HCP 0.125~0.875 共 7 个浓度
    ...
    for i, num_replace in enumerate(num_replace_list):
        ...
        lc_a, lc_c = get_hcp_binary_lc(natoms_list, [element_initial, element_replace])

        atoms = bulk(element_initial, "hcp", a=lc_a, c=lc_c)
        atoms = atoms * (2, 2, 2)
        ...
        xyz_fn = (
            f"{element_initial}{element_replace}_hcp_binary_permutation_conc_{i+1}.xyz"
        )
...
if __name__ == "__main__":
    ...
    for element1, element2 in combinations(elements, 2):
        ...
        print(f"HCP Binary {chemsys}:")

        xyz_fn = f"{element1}{element2}_hcp_binary_permutation.xyz"
        if os.path.exists(xyz_fn):
            os.remove(xyz_fn)
