...
# [ ] 待完善
def self_interstitial_generation(symbols: str, a: float, interstitial_type: str, output_fn: str):
    """BCC、FCC、HCP 结构的自间隙原子构型生成"""

    if "100" in interstitial_type:
        interstitial_type = "<100>dumbbell"
    elif "110" in interstitial_type:
        interstitial_type = "<110>dumbbell"
    elif "111" in interstitial_type:
        interstitial_type = "<111>dumbbell"

    atoms = Atoms(
        symbols=[symbols] * 3,
        cell=Cell.fromcellpar([a, a, a, 90, 90, 90]),
        scaled_positions=bcc_interstital_dict[interstitial_type],
        pbc=True,
    )
