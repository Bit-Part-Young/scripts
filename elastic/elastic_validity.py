#!/usr/bin/env python
"""
验证弹性张量矩阵是否符合 7 种晶系，11 种 Laue 类
主要用于 LAMMPS 计算 弹性性质随点缺陷浓度的影响 project 中
该 project 中的部分弹性张量矩阵数值形式并不是很规整，需进行验证

reference: https://github.com/MineralsCloud/elastic-stability-conditions/blob/master/src/escpy/elasticity.py
...
class StiffnessMatrix:
    ...
    def eps(self, value):
        ...
        if abs(value) > 1:
            warnings.warn(
                "The value of `eps` attribute may be too large!", stacklevel=2
            )
        self._eps = value
    ...
    def validate(self, outfile=None):
        ...
        for criterion, criterion_text in zip(
            self.symmetry_conditions, self.symmetry_conditions_text
        ):
    ...
    def symmetry_conditions(self):
        ...
        if self._stiffness_matrix[0, 4] == 0:  # Rhombohedral (I) class
            return [
                ...
                np.isclose(c[0, 2], c[1, 2], atol=eps),
                np.isclose(c[5, 5], 0.5 * (c[0, 0] - c[0, 1]), atol=eps),
                np.isclose(c[0, 3], -c[1, 3], atol=eps)
                and np.isclose(c[0, 3], -c[4, 5], atol=eps),
            ]
        ...
        return [  # Rhombohedral (II) class
            ...
            np.isclose(c[0, 3], -c[1, 3], atol=eps)
            and np.isclose(c[0, 3], -c[4, 5], atol=eps),
            np.isclose(c[1, 4], -c[0, 4], atol=eps)
            and np.isclose(c[3, 5], -c[0, 4], atol=eps),
        ]
...
class TriclinicSystemStiffnessMatrix(StiffnessMatrix):
    @property
    def symmetry_conditions_text(self):
        return ["There are 21 independent values."]

    @property
    def symmetry_conditions(self):
        c = self._stiffness_matrix
        return [len(np.unique(c)) == 21]
